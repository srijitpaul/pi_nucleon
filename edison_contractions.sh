...
configlist_name='/global/homes/s/srijitp/spaul/projects/pi_nucleon/configstoch.list'
...
#  echo "# [$MyName] number of processes = $nproc" >> $log
  ...
  do
    ...
    srun -n $nproc -N 2  $path_to_qlua/qlua  $include $input $path_to_prog/$QLUA_SCRIPT >edison_contract_$source_location_iterator.out &
  done
done
wait
