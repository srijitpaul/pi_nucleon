...
do
  ...
  do
    ...
    srun -n $nproc -N 2 -c2 --cpu_bind=cores $path_to_qlua/qlua  $include $input $path_to_prog/$QLUA_SCRIPT >edison_contract_$source_location_iterator.out &
  done
done< $scripts/configlist.temp
wait
