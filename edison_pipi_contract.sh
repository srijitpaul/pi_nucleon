...
##
###SBATCH -e edison_piN.err
###SBATCH -o edison_piN.out
...
declare -a momenta_list=("""""""""""""""""""""""""""""""""""""""""""""""""""""""")
params=params.qlua
include="$scripts/plaquette.qlua  $scripts/timer.qlua $scripts/stout_smear.qlua $scripts/load_gauge_field.qlua $scripts/random_functions.qlua $scripts/gamma_perm_phase.qlua $scripts/gi_dcovi.qlua $scripts/make_mg_solver.qlua $scripts/read_propagator.qlua $scripts/deltapp_piN.qlua $scripts/deltaM_piN.qlua $scripts/piN_piN.qlua $scripts/piN_piN_oet.qlua"
...
do
  ...
  src_loc_strng="{$v}"
  echo "$src_loc_strng"
  cat $scripts/pipi_params.qlua | awk '
  /^source_locations/ {print "'"source_locations = $src_loc_strng"'"; next}
  /^nconf/ {print "nconf = ", '$g'; next}
  {print}' > $input
  
  srun -n $nproc -N 1 -c2 --cpu_bind=cores $path_to_qlua/qlua  $include $input $path_to_prog/$QLUA_SCRIPT >edison_contract_$source_location_iterator.out &
done
wait
