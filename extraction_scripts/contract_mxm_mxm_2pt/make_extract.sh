...
">extract_mxm_mxm_1.sh
while read -r conf
        do
	echo "srun -n 729 mpi_parallel_extract_"$conf".out">>extract_mxm_mxm_1.sh
	done<xaa
...
">extract_mxm_mxm_2.sh
while read -r conf
        do
        echo "srun -n 729 mpi_parallel_extract_"$conf".out">>extract_mxm_mxm_2.sh
        done<xab
