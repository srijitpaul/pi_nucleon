...
#pragma omp parallel for private ( phase) shared (m_m)
		...
		for(int gf_num = 0; gf_num <7;++gf_num) {
			for( int gi_num = 0; gi_num <7;++gi_num) {
				...
				for(int i = 0; i < 8;i++) {
					...
					sprintf(buffer_path,"/m-m-lc/gf%.2d_gi%.2d/px%.2dpy%.2dpz%.2d/x%.2dy%.2dz%.2dt%.2d",
	...
	for(int gf_write=0;gf_write<7;gf_write++) {
		for(int gi_write=0;gi_write<7;gi_write++) {
			...
			sprintf(outfilename,"m_m_lc/m-m-lc_g%.2d-g%.2d_pix%.2dpiy%.2dpiz%.2d_pfx%.2dpfy%.2dpfz%.2d",gc[gf_write],gc[gi_write],px,py,pz,-px,-py,-pz); 
			ofstream fout(outfilename,ios::out);
			fout<< setprecision(16);
			fout<<scientific;
			for(int iconf = 0; iconf<44;iconf++) { 
				for(int i = 0; i < 8;i++) {
					sprintf(key,"# /m-m-lc/gf%.2d_gi%.2d/px%.2dpy%.2dpz%.2d/x%.2dy%.2dz%.2dt%.2d",gc[gf_write],gc[gi_write],px,py,pz,source_list[iconf][i][0],source_list[iconf][i][1],source_list[iconf][i][2],source_list[iconf][i][3]);
					fout<<key<<endl;
					for(int it=0; it<T_global; it++) {
